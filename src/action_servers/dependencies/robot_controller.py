...
class robot:
    def __init__(self, robotIP: str, DEBUG: bool=False):
        ...
        # Flag: refresh arm config bits before the next Cartesian move (set after any joint move)
        self._needs_cart_refresh = False
    ...
    def write_joint_pose(self, joint_position_array:list[float], blocking:bool=True):
        ...
        FANUCethernetipDriver.writeJointPositionRegister(self.robot_IP, self.PRNumber, self.CurJointPosList)
        self.start_robot(blocking=blocking)
        self._needs_cart_refresh = True  # arm config may have changed; refresh before next cart move
    ...
    def write_cartesian_position(self, coords:list[float], blocking:bool=True):
        ...
        @param coords[X, Y, Z, W, P, R]  OR  coords[[X,Y,Z,W,P,R], [X,Y,Z,...], ...]
            ...
            - Roll (optional)
        """
        # Refresh arm config bits only when a joint move has occurred since the last
        # Cartesian move. This prevents stale IK config bits from causing unexpected
        # arm flips without flooding the robot with unnecessary EtherNet/IP connections.
        if self._needs_cart_refresh:
            self.CurCartesianPosList = FANUCethernetipDriver.returnCartesianCurrentPostion(self.robot_IP)
            self._needs_cart_refresh = False
